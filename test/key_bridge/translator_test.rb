...
module KeyBridge
  class TranslatorTest < MiniTest::Test
    ...
    def test_array_write_to_new_array
      @map = { 'title' => 'organizations[].title' }
      hash = { title: 'Collator' }

      assert_equal 'Collator',
        translator.translate(hash)[:organizations][0][:title]
    end

    def test_array_write_to_new_sparse_array
      @map = { 'title' => 'organizations[2].title' }
      hash = { title: 'Collator' }

      assert_equal [nil, nil, { 'title' => 'Collator' }],
        translator.translate(hash)[:organizations]
    end
  end
end
